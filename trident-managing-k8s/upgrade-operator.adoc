...
通常情況下，您應該使用與最初安裝 Astra Trident 時相同的方法進行升級。在嘗試使用 Trident operator 進行升級之前，請先檢閱 link:upgrade-trident.html#upgrade-trident.html#step-3-select-an-upgrade-method["選擇升級方法"]。
...
NOTE: 如果狀態顯示 `Updating`，請確保在繼續操作之前解決該問題。有關可能的狀態值列表，請參閱link:../trident-get-started/kubernetes-deploy-operator.html["請按這裡"^]。
...
. （可選）如果需要修改安裝參數，請更新 `TridentProvisioner`規格。這可能包括變更下列設定： `tridentImage`、 `autosupportImage`的值、私人映像儲存庫，以及在刪除命名空間作用域營運者之前、安裝叢集作用域營運者之後提供 `imagePullSecrets`）。如需可更新參數的完整清單，請參閱link:../trident-get-started/kubernetes-customize-deploy.html#configuration-options["組態選項"]。
+
[listing]
----
