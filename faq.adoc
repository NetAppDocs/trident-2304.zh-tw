...
是的，Astra Trident 可以離線安裝。請參閱 link:./trident-get-started/kubernetes-deploy.html["請按這裡"]。
...
Trident 安裝程式會建立一個 Kubernetes ClusterRole，該角色對 Kubernetes 叢集的 PersistentVolume、PersistentVolumeClaim、StorageClass 和 Secret 資源具有特定的存取權限。請參閱 link:./trident-get-started/kubernetes-customize-deploy-tridentctl.html["請按這裡"]。
...
如有需要，您可以本地產生和修改 Astra Trident 安裝時使用的確切清單檔案。請參閱 link:./trident-get-started/kubernetes-customize-deploy-tridentctl.html["請按這裡"]。
...
如果要降級，需要考慮許多因素。請參閱link:./trident-managing-k8s/downgrade-trident.html["降級一節"]。
